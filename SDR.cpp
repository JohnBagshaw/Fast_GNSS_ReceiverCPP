...
static void postProcessing(const Settings_t* settings);
...
#define FUNC_PREFIX "main(): "
  ...
  for (int fileNumber = startFileNumber; fileNumber <= endFileNumber; fileNumber++)
  {
    ...
    * this file contains raw data which is processed by this application*/
    ...
    //sprintf(fileName, "%sMarch 29 with bias.bin", "..\\..\\in_data\\");
    sprintf(fileName, "%sNTLab_Bands_GPS_GLONASS_L12.bin", ".\\mat\\GNSS_signal_records\\");
    printf("Processing File %s \n", fileName);

    /** Initialize settings */
    initSettings(fileName, &globalSettings);
    /** Call main post processing function and calculate time spent in it */
    // gnssStart dictates which processing to run.
...
***********************************/
void postProcessing(const Settings_t* settings) 
{
...
FILE *fid = fopen(settings->fileName, "rb");
  ...
  else
  {
    ...
    if (settings->skipAcquisition == 0)
    {
      ...
      int8_t *data    = new int8_t [(int)BUFFLENGTH]; // 420112
      ...
      // 62    92    80    38   225    91    66    82   144    81
      //1    -1    -1     1    -3    -1    -1    -1     3    -1
...
#if 0
          /*Use channel 2*/
          int8_t ch2DataS8 = (int8_t)((ch2DataU8 >> 1) ? 3 : 1)* (int8_t)((ch2DataU8 & 1) ? -1 : 1);
          dataDbl[i] = (double)ch2DataS8;
          /*Use channel 3*/
          int8_t ch3DataS8 = (int8_t)((ch3DataU8 >> 1) ? 3 : 1)* (int8_t)((ch3DataU8 & 1) ? -1 : 1);
          dataDbl[i] = (double)ch3DataS8;
          /*Use channel 4*/
          int8_t ch4DataS8 = (int8_t)((ch4DataU8 >> 1) ? 3 : 1)* (int8_t)((ch4DataU8 & 1) ? -1 : 1);
...
*/
      ...
      switch (settings->acqMethod)
      {
        ...
        case WEAK_SIGNAL_ACQUISITION:
        {
            ...
            acqResultValid = weak_acquisition(baseBandData,settings,&weakAcqSettings,(WeakAcqResults_t*)&acqResults);
            delete[] baseBandData;
            Toc(4);
        ...
        case HALF_BIT_METHOD:
        {
            disp("Acquiring satellites...");
            Tic(5);
            acqResultValid = half_bit_acquisition(dataDbl,settings,&acqResults,(int)(BUFFLENGTH >> 1));
            Toc(5);
    ...
    //tracking(fid, inChannels, settings, trackResults, outChannels);
    //Toc(6);
    ...
    saveResults(&acqResults, &trackResults[0], settings,false);

    /** Read results from MATLAB and draw comparison with results we got from here... */
    //compareAcquisitionResults(settings, &acqResults, &isMatEqualAcqResults);
    //compareTrackingResults(&trackResults[0], &isMatEqualTrackResults[0], settings);

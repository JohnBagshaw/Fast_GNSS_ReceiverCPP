...
void acquisitionCore(const Settings_t* settings,AcqResults_t* acqResults,
  int    PRNIdx,double* signal1,double* signal2,double* signal0DC,double** caCodeTables,bool* sigFound
)
{
  ...
  int PRN = (int)settings->acqSatelliteList[PRNIdx];
    ...
    for (size_t frqBinIndex = 0; frqBinIndex < numberOfFrqBins; frqBinIndex++)
    {
      ...
      for (size_t i = 0; i < samplesPerCode; i++)
      {
        ...
        acqRes1[i][0] = acqRes1[i][0] * acqRes1[i][0] + acqRes1[i][1] * acqRes1[i][1]; // I^2 + Q^2 for 1st ms
        acqRes2[i][0] = acqRes2[i][0] * acqRes2[i][0] + acqRes2[i][1] * acqRes2[i][1]; // I^2 + Q^2 for 2nd ms
    ...
    if ((peakSize / secondPeakSize) > settings->acqThreshold)
    {
      *sigFound = true;
      ...
      memset(xCarrierOut, 0, sizeof(fftw_complex) * fftNumPts);
      for (size_t i = 0; i < 10 * samplesPerCode; i++)
      {
...
#define FUNC_PREFIX             "acquisition(): "
  ...
  memcpy(signal1, &longSignal[0],                   sizeof(double) * samplesPerCode);
  memcpy(signal2, &longSignal[(int)samplesPerCode], sizeof(double) * samplesPerCode);
  subtractMean(signal0DC, longSignal, BUFFLENGTH);
  ...
  for (size_t i = 0; i < totalThreads; i++)
  {
    for (size_t j = 0; j < numParallelThreads; j++)
    {
      ...
      parallelAcquisitionThreads[PRNIdx] = thread(acquisitionCore,settings,acqResults,PRNIdx,signal1,signal2,signal0DC,caCodeTables,&sigFoundAny[i]);
    }

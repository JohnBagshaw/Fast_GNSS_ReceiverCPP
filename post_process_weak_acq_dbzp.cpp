...
namespace processing
{
	...
	)
	{
        ...
        int32_t numBlocks          = p_processSignal->numBlocks;
        int32_t averFactor         = p_processSignal->averFactor;
        int32_t numSamplesPerBlock = p_processSignal->numSamplesPerBlock;
        int32_t numDopplerFftBins  = p_processSignal->numDopplerFftBins;
        int32_t numCodeSamples     = numSamplesPerBlock * numBlocks;

        double   chipRateHz      = sdrParams.sysParams.caCodeChipRateHz;
        double   intermFreqHz    = dataFileParams->intermFreqHz;
        double   samplingFreqHz  = dataFileParams->samplingFreqHz;
        int32_t  numSamplesPerMs = int32_t(samplingFreqHz * 1e-3);
        int32_t  numChipsPerMs   = int32_t(chipRateHz * 1e-3);
        ...
        int32_t numCodeSamplesDs = numCodeSamples;
        int32_t numDdmSamplesPerPrn = numCodeSamplesDs * numDopplerFftBins;

        int32_t numSamplesCorr = numDdmSamplesPerPrn * numPrns * 2;
        if (sdrParams.stateParams.currFrameNum == 0)
        {
            // Allocate memory and assign ddm processing buffer here
            postProcessResults->numSamplesPerPrn = numDdmSamplesPerPrn * 2;
        ...
        for (int32_t prnIdx = 0; prnIdx < numPrns; prnIdx++)
        {
            ...
            complex<double>* p_ddmMapPrn = &postProcessResults->accumDdm[prnIdx * numDdmSamplesPerPrn * 2];
            ...
            int32_t codeDelay       = floor(peakLoc / numDopplerFftBins);
            int32_t dopplerShiftInt = (peakLoc % numDopplerFftBins);
            ...
            for (int32_t sIdx = 0; sIdx < (numCodeSamplesDs - 19); sIdx++)
            {
                ...
                if (p_ddmMapPrn[dopplerShiftInt + idx * numDopplerFftBins].real() > secondPeakValue)
                {
                    secondPeakValue = p_ddmMapPrn[dopplerShiftInt + idx * numDopplerFftBins].real();
                }
            }
        ...
        // Define FFT Plan
        int32_t fftNumPts = 2 * (1 << int32_t(ceil(log2(numSamplesPerMs))));
        ...
        if (acqPrnCount)
        {
            ...
            for (int32_t aprxIdx = 0; aprxIdx < acqCount; aprxIdx++)
            {
                ...
                // Assigning buffers
                double* p_dopplerSpectrum = new double[numDopplerFftBins];
                complex<double>* p_rxData = new complex<double>[numSamplesPerMs];
                complex<double>* p_rxDataDemod = new complex<double>[numSamplesPerMs];
                complex<double>* p_caCode = new complex<double>[numSamplesPerMs];
                complex<double>* p_cplxExp = new complex<double>[numSamplesPerMs];
                ...
                */
                ...
                for (int32_t dbinIdx = 0; dbinIdx < numDopplerFftBins; dbinIdx++)
                {
                    p_dopplerSpectrum[dbinIdx] = p_ddmMapPrn[codeDelay * numDopplerFftBins + dbinIdx].real();
                }

                // Coarse estimate
                int32_t mOneIdx = (dopplerShiftInt - 1) % numDopplerFftBins;
                int32_t midIdx = (dopplerShiftInt)      % numDopplerFftBins;
                int32_t pOneIdx = (dopplerShiftInt + 1) % numDopplerFftBins;
                ...
                dopplerShiftInt = dopplerShiftInt - floor(numDopplerFftBins / 2);
                ...
                if (rxDataAdjIdx < (rxDataPerFrame->rxDataPerFrameBytesAllocated / sizeof(int32_t)))
                {
                    ...
                    */
                    ...
                    for (int32_t sIdx = 0; sIdx < numSamplesPerMs; sIdx++)
                    {
                    ...
                    for (int32_t sampleIdx = 0; sampleIdx < numSamplesPerMs; sampleIdx++)
                    {
                    ...
                    {
                        lag = maxLag - i;
                        for (int32_t j = 0; j < numSamplesPerMs; j++)
                        {
                            p_corrOut[i] += (p_rxDataDemod[j] * conj(p_caCode[lag]));
                            lag++;
                            if (lag == numSamplesPerMs)
                            {
                                break;
                            }
                        }
                    ...
                    */
                    ...
                    std::fill(placeHolderMem, placeHolderMem + fftNumPts, std::complex<double>(0.0, 0.0));
                    memcpy(placeHolderMem, p_rxData, numSamplesPerMs * sizeof(complex<double>));

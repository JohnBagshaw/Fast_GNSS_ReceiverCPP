...
void initSettings(const char* Nfiles,Settings_t* settings)
{

  int i0;
  // 1 means run post processing, 2 means run DDM processing, else exit
  settings->gnssStart = 2; // DDM processing is enabled because this variable is set to 2.
  ...
  settings->acqMethod = NORMAL_SIGNAL_ACQUISITION;
  ...
  settings->dataType = DATA_TYPE_INT8;

  settings->iF = 14.5799999999999e6;

  // [Hz]
  settings->samplingFreq = 53.0e6;
  //settings->samplingFreq = 38.192e6;
  ////  Intermediate, sampling and code frequencies
  //settings->iF = 9.548E+6;

  //// [Hz]
  //settings->samplingFreq = 3.8192E+7;
